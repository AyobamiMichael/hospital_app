...
class HeartApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: PatientHeartData(),
    );
  }
}

class PatientHeartData extends StatelessWidget {
  const PatientHeartData({super.key});
...
class PatientScreenState extends State<PatientScreen> {
  ...
  static List<String> newlistOfSensorValues = [''];

  @override
  void initState() {
    super.initState();
    // Fetch data from here
    fetchData(WirelessClassState.wifiGateway!);
  ...
  Future<void> fetchData(String wifiGateway) async {
    try {
      ...
      newlistOfSensorValues =
          wirelessSensorValues.map((element) => element.text).toList();
      // print(newlistOfSensorValues[2].substring(22));
    } catch (e) {
      print('Error: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (WirelessClassState.listOfSensorValues.isNotEmpty) {
      ...
      // print(newlistOfSensorValues);
      //print(WirelessClassState.listOfSensorValues[1].substring(17));
      ...
      // breathingRate2 = WirelessClassState.listOfSensorValues[1];
      ...
      // bloodPressure2 = WirelessClassState.listOfSensorValues[0];
...
class HeartVerticalLinePainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, size) {
    canvas.drawLine(Offset(size.width - 190, -315),
        Offset(size.width - 190, size.height - 160), _paintLine);
    ...
    maxLabelSize.layout();
    maxLabelSize.paint(canvas, Offset(size.width - 190, -318));

    // DISPLAY TEXT FOR NAME AND UNIT
    double displayValue = double.parse(heartBeatDisplayValue);
    String heartBeatLabel = '$displayValue';
    final heartDisplayLabelSize = TextPainter(
      text: TextSpan(
          text: heartBeatLabel,
    ...
    heartDisplayLabelSize.layout();
    heartDisplayLabelSize.paint(canvas, Offset(size.width - 150, -50));

    final heartDisplayLabelName = TextPainter(
      text: const TextSpan(
          text: 'Heart wave form',
          style: TextStyle(
              fontStyle: FontStyle.normal,
              fontWeight: FontWeight.bold,
              fontSize: 15)),
    ...
    heartDisplayLabelName.layout();
    heartDisplayLabelName.paint(canvas, Offset(size.width - 130, -340));

    final heartDisplayLabelUnit = TextPainter(
      text: const TextSpan(
          text: 'Bpm',
          style: TextStyle(
              fontStyle: FontStyle.normal,
    ...
    heartDisplayLabelUnit.layout();
    heartDisplayLabelUnit.paint(canvas, Offset(size.width - 100, -60));
    final oxygenInTheBloodDisplayLabel = TextPainter(
      text: const TextSpan(
          text: 'Oxygen in the blood',
          style: TextStyle(
              fontStyle: FontStyle.normal,
              fontWeight: FontWeight.bold,
              fontSize: 15)),
    ...
    oxygenInTheBloodDisplayLabel.layout();
    oxygenInTheBloodDisplayLabel.paint(canvas, Offset(size.width - 130, 30));
    ...
    minLabelSize.layout();
    minLabelSize.paint(canvas, Offset(size.width - 190, -102));
    ...
    zeroLabelSize.paint(canvas, Offset(size.width - 190, -200));
  }
